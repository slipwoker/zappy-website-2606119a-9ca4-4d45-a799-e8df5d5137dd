document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    fields.forEach(function (field) {
      ...
      if (field.hasAttribute('required') && value === '') {
        errorMessage = formatFieldName(name) + ' is required.';
      } else if (value !== '') {
        if (field.type === 'email' && !isValidEmail(value)) {
          ...
          errorMessage = formatFieldName(name) + ' must be at least ' + field.minLength + ' characters.';
        } else if (field.maxLength && field.maxLength > 0 && value.length > field.maxLength) {
          errorMessage = formatFieldName(name) + ' must not exceed ' + field.maxLength + ' characters.';
        } else if (field.pattern) {
          const pattern = new RegExp('^(?:' + field.pattern + ')$');
          if (!pattern.test(value)) {
            errorMessage = formatFieldName(name) + ' format is invalid.';
          }
        }
      }
  ...
  function initScrollAnimations() {
    const elements = document.querySelectorAll(
      '.fade-in, .animate-on-scroll, [data-animate], .section-title, .feature-card, .pricing-card, .testimonial-card, .hero-content, .about-content, .service-item, .team-member, .portfolio-item, .cta-section, .stats-item, .blog-card'
    );
  ...
  const mutationObserver = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      mutation.addedNodes.forEach(function (node) {
        ...
        const animatableSelectors = '.fade-in, .animate-on-scroll, [data-animate]';
        if (node.matches && node.matches(animatableSelectors)) {
          initScrollAnimations();
        } else if (node.querySelector) {
          const children = node.querySelectorAll(animatableSelectors);
          if (children.length) initScrollAnimations();
        }
      });
    });
  });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Ron Rahav (JeepRon) משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '2606119a-9ca4-4d45-a799-e8df5d5137dd',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '2606119a-9ca4-4d45-a799-e8df5d5137dd';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '2606119a-9ca4-4d45-a799-e8df5d5137dd';
